...
let majorEarthquakeData = new L.LayerGroup(); //---------------- Deliverable 2
// 2. Add a reference to the tectonic plates group to the overlays object.
let overlays = {
  "Earthquakes": allEarthquakes,
  "Tectonic Plates" : tectonicPlates, //-------------------------
  "Major Earthquakes" : majorEarthquakeData //----------------- Deliverable 2
};
...
// Looping through our intervals to generate a label with a colored square for each interval.
  ...
  // 3.a. Pass the tectonic plate data to the geoJSON() layer.
  // 3.b. Style the lines with a color and weight that will make it stand out on all maps.
  // 3.c. Add the tectonic layer group variable you created in Step 1 to the map, i.e., .addTo(tectonicPlates) and close the geoJSON() layer.
  // 3.d. Next, add the tectonic layer group variable to the map, i.e, tectonicPlates.addTo(map).
  // 3.e. Finally, close the d3.json() callback.
...
// use the d3.json() callback method to make a call to the major earthquake data from the GeoJSON Summary Feed for M4.5+ Earthquakes
 d3.json("https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/4.5_week.geojson").then(function(majorData) {
// 4. Use the same style as the earthquake data (TECTONIC CODE).
...
// change the getColor() function to use only three colors for the following magnitudes
function getColor(magnitude) {
  if (magnitude < 5 ) {
    return "#FFF200"; 
  }
...
//  after the marker has been created and styled.
// 7.a. Turn each feature into a circleMarker on the map
// 7.b. Style each circle with styleInfo() function
// 7.c. Create a popup for the circle to display the magnitude and location of the earthquake
// 7.d. Add the major earthquake layer group variable you created in Step 1 to the map, i.e., .addTo(majorEQ) and then close the geoJSON() layer.
L.geoJson(majorData, {
  pointToLayer: function(feature, latlng) {
    return L.circleMarker(latlng);
  },
